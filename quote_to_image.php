...
function fitText($quote_array, $width, $height, $font_size, $timestringStarts, $timestring_wordcount, $margin) {
    ...
    foreach($quote_array as $key => $word) {
		if ($word[0] === '*') {
            $italic = 1;
            // Check if it’s a single italic word
            // In this case the last letter is a * as well.
        ...
        // disable italic after a single italic word.
        if ($disableItalic === 1) {
            $disableItalic = 0;
            $italic = 0;
        }
        ...
        if ( ($position[0] + $textwidth) >= ($width - $margin) ) {
            ...
            $position[0] = $margin;
            $position[1] = $position[1] + round($font_size*1.414); // 'golden ratio' line height
